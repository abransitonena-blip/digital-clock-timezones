"""Genera todas las salidas de fabricación (se ejecuta dentro del contenedor de KiCad 9).

    python3 gen/outputs.py

Crea hardware/controlador-flechas-36v/fabricacion/ con:
...
PCB = os.path.join(KI, "ControladorFlechas.kicad_pcb")
SCH = os.path.join(KI, "ControladorFlechas.kicad_sch")
...
def main():
    ...
    # ---- esquema y ERC ----
    run("kicad-cli", "sch", "export", "pdf", "-o", os.path.join(esq, "ControladorFlechas_esquema.pdf"), SCH)
    run("kicad-cli", "sch", "erc", "--severity-all", "-o", os.path.join(rep, "ERC_ControladorFlechas.rpt"), SCH)
    run("kicad-cli", "sch", "export", "bom", "-o", os.path.join(bom, "BOM_KiCad.csv"),
        "--fields", "Reference,Value,Footprint,${QUANTITY},${DNP},Funcion",
        "--labels", "Referencia,Valor,Huella,Cantidad,No montar,Funcion",
        "--group-by", "Value,Footprint", "--ref-range-delimiter", "", SCH)
    run("kicad-cli", "sch", "export", "netlist", "-o", os.path.join(esq, "ControladorFlechas.net"), SCH)

    # ---- DRC con paridad ----
    run("kicad-cli", "pcb", "drc", "--schematic-parity", "--severity-all", "-o",
        os.path.join(rep, "DRC_ControladorFlechas.rpt"), PCB)
    ...
    # ---- renders 3D ----
    common = ["--quality", "basic", "--width", "1800", "--height", "1400", "--background", "transparent"]
    ...
    run("kicad-cli", "pcb", "render", "-o", os.path.join(d3, "render_perspectiva.png"), "--side", "top",
        "--perspective", "--rotate", "-50,0,25", "--zoom", "1.1", *common, PCB)
    run("kicad-cli", "pcb", "export", "step", "-o", os.path.join(d3, "ControladorFlechas.step"),
